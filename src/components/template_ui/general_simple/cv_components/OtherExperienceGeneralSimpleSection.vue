<!-- src/components/ProjectGeneralSimpleSection.vue -->
<template>
      ...
      <div class="session-item" v-for="(other, index) in otherExperienceList" :key="index" @mouseenter="handleMouseEnter(index)"
        ...
        <div class="item-hover-overlay" v-if="hoverIndex === index">
          ...
          <div class="overlay-buttons">
            ...
            <button class="overlay-button ai-dialog-button" @click="onTitleClick('otherExperience', other.title)">AI
              对话</button>
            <button class="overlay-button delete-button"
              @click="onTitleDelete('otherExperience', other.title)">删除</button>
          </div>
        </div>
  ...
  <script>
  export default {
    name: "OtherExperienceGeneralSimpleSection",
    props: {
      otherExperienceList: {
        type: Array,
        required: true
    ...
    mounted() {
      console.log(this.otherExperienceList);
    },
    methods: {
