    ...
    <div class="session-item" v-for="(project, index) in projectList" :key="index" @mouseenter="handleMouseEnter(index)"
      ...
      <div class="item-hover-overlay" v-if="hoverIndex === index">
        ...
        <div class="overlay-buttons">
          ...
          <button class="overlay-button ai-dialog-button" @click="onTitleClick('projectExperience', project.title)">AI
            对话</button>
          <button class="overlay-button delete-button"
            @click="onTitleDelete('projectExperience', project.title)">删除</button>
        </div>
      </div>
...
<script>
export default {
  name: "ProjectGeneralSimpleSection",
  props: {
    projectList: {
      type: Array,
      required: true
